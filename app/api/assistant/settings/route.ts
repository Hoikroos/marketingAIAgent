...
export async function GET() {
  ...
  return NextResponse.json({ ok: true, settings, canEdit: !!user && isAdminLike(user) });
}
...
export async function PUT(req: NextRequest) {
  ...
  await logActivity("assistant", `Cập nhật cài đặt Trợ lý AI (model=${model || "mặc định"}, temp=${temperature}, maxTokens=${maxTokens}, dữ liệu hệ thống=${useData ? "bật" : "tắt"})`);

  return NextResponse.json({ ok: true, settings: { model, temperature, maxTokens, useData } });
}
