...
| **Lead** | name, phone?, source, status, purpose?, contacted, responseStatus, utmSource/Medium/Campaign/Content/... | Khách hàng tiềm năng — **KHÔNG có người phụ trách** (đã bỏ `ownerId`/`owner`) |
...
| **SocialMetric** | platform, channel, followers, views, engagement, **videosPosted** (số video đã đăng trong tuần), weekLabel ("YYYY-Wxx"), note?, ownerId? | Số liệu MXH nhập theo tuần; **followers = ảnh chụp follow HIỆN TẠI của kênh**, còn views/engagement/videosPosted = số **phát sinh trong tuần**; unique [platform, channel, weekLabel, ownerId] |
...
| `/api/work-reports/general` | **BÁO CÁO CHUNG tháng**: GET trả dữ liệu tổng hợp theo tuần/nhân viên (Dashboard) kèm 2 cờ quyền `canSynthesize`/`canExportWord`; POST `{year, month, mode: "synthesize"\|"word"}` — "synthesize" chạy AI tổng hợp (lib/monthlyReport.ts) + cache vào Setting `generalReport_<Y>-<MM>` (quyền `reports_work_synthesize`), "word" trả file .docx (lib/docx.ts tự dựng ZIP, nội dung xuống hàng theo từng câu/chỉ số) dùng lại cache AI có sẵn — KHÔNG gọi AI (quyền `reports_work_export_general`) | GET/POST |
...
- **Phân quyền** (`lib/permissions.ts`):
  ...
  - Quyền mặc định nhân viên mới: mọi action của STAFF_MODULES, trừ `DEFAULT_EXCLUDED` (team_update, team_delete, ads_update, các thao tác ghi/xoá/xuất của social, `reports_work_synthesize` + `reports_work_export_general` của Báo cáo chung tháng...). Admin cấp thêm trong Cài đặt → Phân quyền.
...
- **Khách hàng tiềm năng KHÔNG còn "Người phụ trách"** — bỏ hẳn khỏi UI + DB: model `Lead` không còn `ownerId`/`owner` (và `User` không còn `leads Lead[]`); `components/LeadsTable.tsx` không còn cột "Phụ trách" + không có select gán người trên từng dòng; `components/CreateLeadForm.tsx`/`CreateLeadModal.tsx` không còn mục "Phân công" và không nhận prop `users` (2 trang `/dashboard/leads`, `/dashboard/leads/new` cũng bỏ `getStaffUsers()`); `/api/leads` (GET/POST/PATCH) không nhận `ownerId`; file Excel xuất ra bỏ cột "Người phụ trách" (còn 9 cột, `MergeAcross="8"`); toggle "Có lead được phân cho nhân viên" (`notifyLead`) đã bỏ khỏi Cài đặt + `app/api/settings/route.ts`. **KHÔNG còn THÔNG BÁO NÀO VỀ LEAD**: job "Follow-up lead cũ" trong `lib/automation.ts` đã xoá hẳn (các job còn lại đánh số lại 1–5), kèm setting `followUpDays` (Cài đặt → Tự động hoá, `app/api/settings/route.ts`, `prisma/seed.ts`), trường `followups` trong `AutomationResults` (nên `instrumentation.ts`, `app/api/automation/run/route.ts` + toast "Chạy ngay" cũng bỏ), và nhánh `type === "lead"` khi click thông báo ở `components/header.tsx`; thông báo cũ trong DB (type `lead`/`followup`) đã xoá. **Báo cáo chung tháng** (`lib/monthlyReport.ts` + `components/GeneralReportPanel.tsx`) chỉ còn tổng hợp "khách hàng mới" theo **NHÓM/tuần** — không tách được theo từng nhân viên (trước đây dựa vào `Lead.ownerId`).
- **Báo cáo chung tháng** (`lib/monthlyReport.ts` + `/api/work-reports/general` + `components/GeneralReportPanel.tsx` trên Dashboard): gom theo tuần của tháng — báo cáo công việc nộp (đọc text .docx bằng `extractDocxParagraphs` từ `lib/docx.ts`, file lưu StoredFile), nhật ký ngày (`DailyReport.date` dạng "YYYY-MM-DD"), nội dung/lead/task/MXH. AI tổng hợp từng nhân viên (prompt + parse nhãn `[TUẦN ...]`) với fallback mẫu tự động khi thiếu key/lỗi (`fallbackCache()` dựng bản không gọi AI); kết quả cache ở Setting key `generalReport_<Y>-<MM>` (không thêm model mới).
  - **Phân quyền 2 NÚT (tách riêng)**: `canSynthesizeGeneralReport(user)` → nút "Tổng hợp bằng AI" (quyền `reports_work_synthesize`, tốn chi phí AI); `canExportGeneralReportWord(user)` → nút "Tải file Word" (quyền `reports_work_export_general`); xem tổng hợp toàn phòng là `canViewGeneralReportAll(user)` (thêm `dailyreports_viewall`). Cả 3 hàm ở `lib/permissions.ts`, dùng chung cho route API + `app/dashboard/page.tsx` (không lệch quyền giữa UI và API). Admin/quản lý (`isAdminLike`/`users`/`reports_work_create`) mặc định có cả 2; nhân viên mới KHÔNG được cấp (nằm trong `DEFAULT_EXCLUDED`), Admin cấp riêng trong Cài đặt → Phân quyền. Người CHỈ có quyền tải Word sẽ dùng lại bản AI đã cache (hoặc mẫu tự động) — route KHÔNG gọi AI để tránh phát sinh chi phí.
  - **Hiển thị XUỐNG HÀNG (dễ đọc)**: `lib/textLines.ts` (`splitSentences`) tách văn bản tổng hợp thành từng câu/từng mục → panel hiển thị mỗi câu 1 dòng (`TextLines`), mỗi chỉ số 1 dòng (`StatLines`), mỗi file báo cáo nộp 1 dòng; file Word cũng vậy (mỗi câu 1 đoạn `body`, mỗi chỉ số 1 đoạn `bullet`) — module thuần nên dùng chung được cho cả server và client. File Word tạo bằng `buildDocx` — kiểm chứng bằng `scripts/test-docx.ts` và `scripts/test-textlines.ts` (`npx tsx scripts/test-textlines.ts`).
- **Báo cáo Mạng xã hội trên Dashboard** (`lib/socialStats.ts` + `components/SocialReportPanel.tsx`): khối mới nằm ngay dưới biểu đồ "Hiệu suất MXH theo tuần", hiển thị **lượt follow, video đã đăng, view TB/video, tổng view, tương tác** theo TUẦN (8 tuần gần nhất) / THÁNG (6 tháng gần nhất) — tab Tuần/Tháng + chip chọn chỉ số để vẽ biểu đồ, bảng chi tiết từng kỳ (kèm % so với kỳ trước) và bảng "số liệu mới nhất theo kênh". Server gom số bằng hàm THUẦN rồi truyền props xuống client (giống `LeadsTimeStats`) nên không lệch số server/client; khối chỉ hiện khi có quyền `social`.
  - Quy ước số liệu: `followers` = ảnh chụp **MỚI NHẤT** của mỗi kênh trong kỳ (KHÔNG cộng dồn qua các tuần) — KPI "Followers MXH" trên Dashboard trước đây cộng dồn sai đã sửa theo cách này; `videosPosted`/`views`/`engagement` cộng dồn theo kỳ; `avgViews = views ÷ videosPosted` (0 nếu kỳ không có video). Tháng = gộp các tuần có ngày trong tháng (tuần ở ranh giới 2 tháng được tính cho cả 2 tháng — giống trang Trạng thái MXH). Tuần dùng chung `periodKey/periodLabel` của `lib/workStats.ts`. Nếu sau này đổi cách nhập số liệu MXH (vd view là số luỹ kế) thì phải sửa `lib/socialStats.ts`. Kiểm chứng: `npx tsx scripts/test-socialstats.ts`.
